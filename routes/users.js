...
router.route('/login').post((req, res) => {
	...
	if (error != '') {
		...
		if (error != '') {
			...
			User.find({ userName: credential }).then((user) => loginUser(user, password, req, res)).catch((err) =>
				res.status(500).json({
					status: failure,
		...
		User.find({ email: credential }).then((user) => loginUser(user, password, req, res)).catch((err) =>
			res.status(500).json({
				status: failure,
